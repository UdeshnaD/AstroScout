...
export async function getWeatherForSpot(
  ...
  try {
    ...
    return normaliseOpenMeteo(data, startTime) ?? fallbackWeather(spot.id, startTime);
  } catch {
    return fallbackWeather(spot.id, startTime);
  }
}

function normaliseOpenMeteo(data: OpenMeteoResponse, startTime: string): WeatherSummary | null {
  ...
  const nearestIndex = hourly.time.reduce((bestIndex, time, index) => {
    const bestDelta = Math.abs(parseForecastTime(hourly.time?.[bestIndex] ?? "") - target);
    const nextDelta = Math.abs(parseForecastTime(time) - target);
    return nextDelta < bestDelta ? index : bestIndex;
  }, 0);

  const window = hourly.time.slice(nearestIndex, nearestIndex + 5).map((time, offset) => {
    ...
    return {
      ...
      visibilityKm: Math.round(((hourly.visibility?.[index] ?? 18000) / 1000) * 10) / 10,
      precipitationChance: clamp(hourly.precipitation_probability?.[index] ?? 10)
    };
  });

  const cloudCover = clamp(hourly.cloud_cover?.[nearestIndex] ?? 45);
  const visibilityKm = Math.round(((hourly.visibility?.[nearestIndex] ?? 18000) / 1000) * 10) / 10;

  return {
    cloudCover,
    visibilityKm,
    precipitationChance: clamp(hourly.precipitation_probability?.[nearestIndex] ?? 10),
